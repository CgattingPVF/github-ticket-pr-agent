...
class JobStore:
    ...
    def list_tickets(self, limit: int | None = None, state: str = 'OPEN') -> list[dict]:
        ...
        if state:
            query += (
                " WHERE upper(state) = upper(?) AND ("
                " (project_number = 11 AND lower(trim(project_status)) IN ('intake', 'backlog'))"
                " OR (project_number = 6 AND lower(trim(project_status)) IN ('to triage', 'backlog'))"
                ")"
            )
            params.append(state)
